...
def make_alembic_config(
    ...
    base_path: str = str(PROJECT_PATH),
) -> Config:
...
def alembic_config_from_url(pg_url: Optional[str] = None) -> Config:
    ...
    return make_alembic_config(cmd_options, str(PROJECT_PATH.parent))
...
def load_migration_as_module(file: str):
    ...
    return importlib.machinery.SourceFileLoader(
        file, os.path.join(PROJECT_PATH.parent, "alembic", "versions", file)
    ).load_module()
